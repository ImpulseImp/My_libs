...
int main(void)
{
    ...
    while (1) 
    {
		//adcValue = ADCW; // pseudo-register that allows get the data rom ADCL and ADCH at once.
		adcValueLow = (int)ADCL;
		adcValue = (int)ADCH*256;
		adcValue = adcValue + adcValueLow;
		printf("ADC is:%d \n", adcValue);			
    }
}
